...
2. The EMS is then split into a number of shares which are encoded as a set of mnemonic words.
...
To recover a SLIP39 secret, you need a predefined number of words (known as a threshold) from all the words.
...
On the other hand, SLIP39 splits the passphrase multiple words, but a pre-defined amount of words is required to recover the secret. For example, you can select 20 words, with only 15 required to recover the wallet. This way you can store the words individually or in sets of multiple words. If someone finds one of the backup locations, they are not able to recover your wallet. Additionally, if one of the locations is destroyed, you are still able to recover the wallet using the remaining locations.
